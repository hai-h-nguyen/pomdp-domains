...
class CarEnv(gym.Env):
    def __init__(self, seed=0, info_offset=0, rendering=False):
        ...
        if self.obs_type == IMAGE_NORMED_FLATTEN:
            ...
            self.observation_space = gym.spaces.Box(
                shape=(64, 64, 3), low=0, high=255.0, dtype=np.float32
            )
        else:
            self.observation_space = gym.spaces.Box(
    ...
    def _prepare_obs(self, state, goal=None):
        """observation is a three channel image
        ...
        assert self.grid_len < 64
        whole_obs = np.zeros((3, 64, 64))

        start = 32 - self.grid_len // 2
        stop = start + self.grid_len
