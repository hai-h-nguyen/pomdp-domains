...
class LightHouseEnvironment(gym.Env):
    EMPTY = 0
    GOAL = 1
    WRONG_CORNER = 2
    WALL = 3
    ...
    def __init__(self, world_dim: int, world_radius: int, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(
            low=min(self.SPACE_LEVELS),
            high=max(self.SPACE_LEVELS),
            shape=(
                len(
        ...
        self.action_space = gym.spaces.Discrete(2 * self.world_dim)
    ...
    def step(self, action: int) -> Tuple:
        ...
        if np.all(self.current_position == self.goal_position):
