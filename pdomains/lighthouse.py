...
class LightHouseEnv(gym.Env):
    ...
    def __init__(self, world_dim: int = 1, view_radius=1, world_radius: int = 15, expert_view_radius: int = 15, max_step: int = 100, **kwargs):
    ...
    def query_state(self) -> np.array:
        if self.env.goal_position[0] == 0:
            return np.array([0])
        else:
            return np.array([1])

        # return np.array([self.env.current_position[0], self.env.goal_position[0]])
